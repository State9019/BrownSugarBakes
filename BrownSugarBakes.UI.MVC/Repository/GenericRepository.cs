...
namespace BrownSugarBakes.UI.MVC.Repository
{
    public class GenericRepository<Tbl_Entity> : IRepository<Tbl_Entity> where Tbl_Entity : class
    {
        ...
        private dbBrownSugarBakesEntities _DBEntity;

        public GenericRepository(dbBrownSugarBakesEntities DBEntity)
        {
            _DBEntity = DBEntity;
        ...
        public void UpdateByWhereClause(Expression<Func<Tbl_Entity, bool>> wherePrdict, Action<Tbl_Entity> ForEachPredict)
        {
            _dbSet.Where(wherePrdict).ToList().ForEach(ForEachPredict);
        }
    }
}